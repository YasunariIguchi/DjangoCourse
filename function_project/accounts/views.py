...
def user_login(request):
    ...
    if login_form.is_valid():
        ...
        if user:
            if user.is_active:
                login(request, user)
                messages.success(request, "ログインできたお")
                return redirect("accounts:home")
            else:
                messages.warning(request, "ユーザーがアクティブでないお")
        else:
            messages.warning(request, "メールアドレスかパスワードが間違ってるお")
...
def user_logout(request):
    logout(request)
    messages.success(request, "ログアウトしたお")
    return redirect("accounts:home")
...
def user_edit(request):
    ...
    if user_edit_form.is_valid():
        messages.success(request, "更新したお")
        user_edit_form.save()
...
def change_password(request):
    ...
    if password_change_form.is_valid():
        try:
            ...
            messages.success(request, "パスワード更新したお")
